...
To run the tests, enter the phoenix/tests directory and run the following commands::
...
application stacks on one host, remove the port setting entirely and add `VIRTUAL_HOST=farmguru.ansgax.com` to your env file.
This pass all incoming requests on `nginx-proxy` to the nginx service your application is using.

.. _nginx-proxy: https://github.com/jwilder/nginx-proxy

Postgres is saving its database files to `/data/phoenix/postgres` by default. Change that if you wan't
...
Place it in `/etc/supervisor/conf.d/phoenix.conf` and run::

    supervisorctl reread
    supervisorctl start phoenix
